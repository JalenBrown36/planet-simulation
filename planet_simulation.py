...
def add_planet(planets):
    ...
    if planet_index < len(PLANETS_ORDER):
        ...
        new_planet = Planet(x * Planet.AU, 0, size *
                            RADIUS_SCALE, color, mass, isSun)
...
def remove_planet(planets):
    ...
    if planet_index > 1:
        ...
        Planet.SCALE = Planet._initial_scale * \
            (1 / (Planet._planet_count ** .9))
    else:
...
def main():
    ...
    # Create buttons
    add_button = Button(20, 20, 160, 40, "Add Planet",
                        lambda: add_planet(planets))
    remove_button = Button(20, 70, 160, 40, "Remove Planet",
                           lambda: remove_planet(planets))
